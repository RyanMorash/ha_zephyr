...
Do **not** use `verify=False`. Capture the chain once and pin it:

```bash
openssl s_client -showcerts -connect zephyr-prod-app.gemteks.com:443 \
  -servername zephyr-prod-app.gemteks.com </dev/null 2>/dev/null \
  | awk '/BEGIN CERT/,/END CERT/' > gemtek-chain.pem
```

```python
requests.post(..., verify="gemtek-chain.pem")
```

Verified working. The leaf expires **2026-10-15**, so the pin needs
refreshing then — build in a clear failure message for that.
...
6. **Cert pin expiry** 2026-10-15 (§4).
